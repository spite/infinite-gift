...
class Paper extends WrappingPaper {
  ...
  constructor(w, h) {
    ...
    path1.moveTo(.5 * width, 0 + p);
    path1.lineTo(width - p, height - p);
    path1.lineTo(0 + p, height - p);
    path1.lineTo(.5 * width, 0 + p);

    const path2 = new Path2D();
    path2.moveTo(.5 * width, height - p);
    path2.lineTo(width - p, 0 + p);
    path2.lineTo(0 + p, 0 + p);
    path2.lineTo(.5 * width, height - p);
    ...
    function draw(x, y) {
      ...
      this.colorCtx.save();
      this.colorCtx.translate((x + .5) * width, (y) * height);
      this.colorCtx.fillStyle = colorPalette[~~(Math.random() * colorPalette.length)];
      ...
      this.roughnessCtx.save();
      this.roughnessCtx.translate((x + .5) * width, (y) * height);
      this.roughnessCtx.fillStyle = specPalette[~~(Math.random() * specPalette.length)];
