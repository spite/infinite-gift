...
class Paper extends WrappingPaper {
  constructor(w, h) {
    ...
    const colorPalette1 = ["#d24b58", "#c34e55", "#cbe2ee", "#ecf0f3"];
    ...
    function draw(x, y) {
      ...
      this.colorCtx.fill(path);
      this.colorCtx.restore();
      const colors = colorPalette.sort((a, b) =>
        Math.random() > 0.5 ? 1 : -1
      );
      let inc = 0.5 * 0.25;
      let scale = 0.75;
      ...
      this.roughnessCtx.fill(path);
      this.roughnessCtx.restore();
      const colors2 = specPalette.sort((a, b) =>
        Math.random() > 0.5 ? 1 : -1
      );
      inc = 0.5 * 0.25;
      scale = 0.75;
